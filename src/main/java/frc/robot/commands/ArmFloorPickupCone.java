...
public class ArmFloorPickupCone extends CommandBase {
	...
	private final double targetPivotPos = PivotConstants.floorPickupConeAngle;
	private final double targetWristPos = WristConstants.floorPickupConeAngle;
	...
	public void execute() {
		...
		if(pivotCounter < Constants.pidTimer){
			...
			pivot.setCurrentPosition(PositionsPivot.FLOOR_CONE);
		}
		if((Constants.autoTimer.get()-startTime)>1){
			wrist.position(targetWristPos);
			wrist.setCurrentPosition(PositionsWrist.FLOOR_CONE);
		}
	}
