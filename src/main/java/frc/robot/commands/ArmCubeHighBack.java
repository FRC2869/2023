...
public class ArmCubeHighBack extends CommandBase {
	...
	private final double targetPivotPos = PivotConstants.highCubeBackAngle;
	private final double targetWristPos = WristConstants.highCubeBackAngle;
	...
	public void execute() {
		...
		if(pivotCounter < Constants.pidTimer){
			...
			pivot.setCurrentPosition(PositionsPivot.HIGH_CUBE_BACK);
		}
		if((Constants.autoTimer.get()-startTime)>1){
			wrist.position(targetWristPos);
			wrist.setCurrentPosition(PositionsWrist.HIGH_CUBE_BACK);
		}
	}
