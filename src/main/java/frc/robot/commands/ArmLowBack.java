...
public class ArmLowBack extends CommandBase {
	...
	private final double targetPivotPos = PivotConstants.lowBackAngle;
	private final double targetWristPos = WristConstants.lowBackAngle;
	...
	public void execute() {
		...
		if(pivotCounter < Constants.pidTimer){
			...
			pivot.setCurrentPosition(PositionsPivot.LOW_BACK);
		}
		if((Constants.autoTimer.get()-startTime)>1){
			wrist.position(targetWristPos);
			wrist.setCurrentPosition(PositionsWrist.LOW_BACK);
		}
	}
