...
public class ArmSingleSubStationCube extends CommandBase {
	...
	private final double targetPivotPos = PivotConstants.singleSubstationCubeAngle;
	private final double targetWristPos = WristConstants.singleSubstationCubeAngle;
	...
	public void execute() {
		...
		if(pivotCounter < Constants.pidTimer){
			...
			pivot.setCurrentPosition(PositionsPivot.SINGLE_CUBE);
		}
		if((Constants.autoTimer.get()-startTime)>1){
			wrist.position(targetWristPos);
			wrist.setCurrentPosition(PositionsWrist.SINGLE_CUBE);
		}
	}
